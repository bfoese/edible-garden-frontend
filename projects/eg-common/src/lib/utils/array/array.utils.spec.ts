...
describe('ArrayUtils', () => {
  ...
  describe('contains', () => {
    it('should always return false if the array is undefined/null', () => {
      expect(ArrayUtils.contains(null, null)).toBe(false);
    });
    it('should always return false if the array is empty', () => {
  ...
  describe('pushDistinct', () => {
    it('should handle null gracefully', () => {
      expect(ArrayUtils.pushDistinct(null, null)).toBeNull();
    });
    it('should handle undefined gracefully', () => {
      expect(ArrayUtils.pushDistinct(undefined, undefined)).toBeUndefined();
    });
    it('should always return null/undefinded when the source array is null/undefined', () => {
      expect(ArrayUtils.pushDistinct(null, 'A')).toBeNull();
    });
    ...
    it('should push an object with another identity but same content of an already contained one', () => {
      ...
      expect(ArrayUtils.pushDistinct(srcArray, obj2)).toEqual([
        obj1,
        'B',
        obj2
      ]);
  ...
  describe('filterDistinct', () => {
    it('should handle null gracefully', () => {
      expect(ArrayUtils.filterDistinct(null)).toBeNull();
    });
    it('should handle undefined gracefully', () => {
      expect(ArrayUtils.filterDistinct(undefined)).toBeUndefined();
    });
    it('should remove duplicate entries', () => {
      expect(ArrayUtils.filterDistinct(['A', 'B', 'C', 'A', 'B'])).toEqual([
        'A',
        'B',
        'C'
      ]);
